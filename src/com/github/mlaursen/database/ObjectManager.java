...
/**
 * A Database ConnectionManager for a database object. This generates a Database
 * Package with Database Procedures. For each Interface that the Database object
 * implements, it will create a procedure within the package.
 * 
 * @see Package
 * @see Procedure
 * @see Getable
...
public class ObjectManager {
	...
	/**
	 * This adds each procedure to the package for every database objecttype
	 * that the database object implments. It would be nice to just dynamically
	 * create for every class in: com.github.mlaursen.database.objecttypes
	 * 
	 */
	...
	/**
	 * Executes a database stored procedure that does not have a result set as a
	 * result. The return type will be a boolean if the stored procedure
	 * affected at least 1 row. Before executing the stored procedure, a check
	 * is performed to see if that procedure exists in the package, if it does
	 * not exist, false is returned and an error message is printed to the
	 * System.err.
	 * 
	 * @param procedureName
	 *            The procedure name to lookup and call from the Database
	 *            Object's package
	 * @param An
	 *            array of object parameters to be passed to the stored
	 *            procedure. The parameters will be cast to their corresponding
	 *            types. {@link com.github.mlaursen.database.ConnectionManager}
	 * @return A boolean of success for the stored procedure call. If at least 1
	 *         row was affected, the result is true.
	 */
	 ...
	 * Before executing the stored procedure, a check is performed to see if
	 * that procedure exists in the package, if it does not exist, an empty
	 * ResultSet is returned and an error message is printed to the System.err.
	 * 
	 * @param procedureName
	 *            The procedure name to lookup and call from the Database
	 *            Object's package
	 ...
	 *            procedure. The parameters will be cast to their corresponding
	 *            types. {@link com.github.mlaursen.database.ConnectionManager}
	 * @return a MyResultSet for the resulting rows from the database call. @see
	 *         MyResultSet
	 */
	 ...
	 *            Object's package
	 * @param parameters
	 *            An array of object parameters to be passed to the stored
	 *            procedure. The parameters will be cast to their corresponding
	 *            types. {@link com.github.mlaursen.database.ConnectionManager}
	 * @return a MyResultRow for the resulting row from the database call. @see
	 *         MyResultRow
	 */
	 ...
	 * 
	 * @param c
	 *            This is a class within the com.github.database.objecttypes
	 *            package. It creates a procedure based on the lowercase version
	 *            of the class name and removes the 'able' from the name.
	 */
	...
	/**
	 * Gets an array of String parameters from a class based on the DatabseField
	 * annotations given in all super classes and current class
	 * 
	 * @param c
	 * @return
	 */
	...
	/**
	 * This adds a custom procedure that is not automattically generated by
	 * implementing the database object types
	 * 
	 * @param p
	 *            The procedure to add
	 */
	public void addCustomProcedure(Procedure p) {
		this.pkg.addProcedure(p);
	 ...
	 * @param params
	 */
	public void setProcedureParameters(Procedure p, String... params) {
		setProcedureParameters(p.getDisplayName(), params);
	}
